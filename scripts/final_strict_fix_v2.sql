-- Explicitly use public schema to ensure visibility
CREATE OR REPLACE FUNCTION public.get_serial_history_strict(search_term text)
RETURNS TABLE (
    source_type text,
    id uuid,
...
-- Permissions
ALTER FUNCTION public.get_serial_history_strict(text) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.get_serial_history_strict(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_serial_history_strict(text) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_serial_history_strict(text) TO anon;
