-- NEW RPC with unique name to avoid cache/collision issues
CREATE OR REPLACE FUNCTION get_serial_history_strict(search_term text)
RETURNS TABLE (
    source_type text,
    id uuid,
...
ALTER FUNCTION get_serial_history_strict(text) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION get_serial_history_strict(text) TO authenticated;
GRANT EXECUTE ON FUNCTION get_serial_history_strict(text) TO service_role;
GRANT EXECUTE ON FUNCTION get_serial_history_strict(text) TO anon;
