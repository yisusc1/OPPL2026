...
BEGIN
    ...
    SELECT 
        ...
        jsonb_build_object(
            ...
            'user_last_name', COALESCE(p.last_name, ''),
    ...
    WHERE 
        (t.serials IS NOT NULL AND (
            (jsonb_typeof(t.serials) = 'array' AND t.serials @> jsonb_build_array(search_term)) OR
            (jsonb_typeof(t.serials) = 'string' AND t.serials::text LIKE '%' || search_term || '%')
        ))
    ...
    WHERE 
        (item.serials IS NOT NULL AND (
            (jsonb_typeof(item.serials) = 'array' AND item.serials @> jsonb_build_array(search_term)) OR
            (jsonb_typeof(item.serials) = 'string' AND item.serials::text LIKE '%' || search_term || '%')
        ))
