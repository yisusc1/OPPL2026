-- Drop logic to ensure clean replace
...
BEGIN
    RETURN QUERY
    -- 1. Transactions (STRICT SERIAL MATCH)
    SELECT 
        ...
        jsonb_build_object(
            ...
            'user_first_name', COALESCE(p.first_name, 'Sistema'), -- Fallback to 'Sistema' if null, but distinguish in UI? 
    ...
    WHERE t.serials @> jsonb_build_array(search_term) -- THIS IS THE KEY FIX
    
    UNION ALL
    
    -- 2. Assignments (Active/History) - Detailed Serial Match
    SELECT 
